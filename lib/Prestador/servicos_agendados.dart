...
class ServicosAgendadosScreenState extends State<ServicosAgendadosScreen> {
  ...
  Future<void> cancelarServico(String docId, {String? motivo}) async {
    try {
      final String novoStatus = 'cancelada_prestador'; // ← CORRETO
      final String motivoFinal = (motivo ?? '').trim();

      // 🔒 Validação adicional
      if (docId.isEmpty) {
        debugPrint('❌ ERRO: docId está vazio, cancelamento abortado.');
        return;
      }

      print('🔥 CANCELANDO SERVIÇO: $docId com status: $novoStatus');

      // 🔥 Atualiza o documento principal
      await db.collection(_colSolicitacoes).doc(docId).update({
        'status': novoStatus, // ← Isso deve fazer o card sumir
        'canceladaEm': FieldValue.serverTimestamp(),
        'motivoCancelamento': motivoFinal,
        'atualizadoEm': FieldValue.serverTimestamp(),
      });

      // 🔹 UID opcional — evita null check crash
      final uid = auth.currentUser?.uid ?? 'desconhecido';
      ...
      debugPrint('✅ Serviço $docId cancelado com sucesso por $uid');
    } catch (e, st) {
      debugPrint('❌ Erro ao cancelar serviço: $e');
      ...
      builder: (context, snapTodos) {
        ...
        return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapAtivos) {
            ...
            // Filtra cancelados feitos PELO CLIENTE e mantém por 3 dias
            // ============ NO MÉTODO build - substitua a parte do filtro ============

            // Filtra cancelamentos feitos PELO CLIENTE e mantém por 3 dias
            final docsCanceladosFiltrados = docsCancelados.where((doc) {
              final d = doc.data();
              final status = (d['status'] ?? '').toString().toLowerCase();

              // ❌ REMOVE TODOS os cancelamentos do prestador
              if (status.contains('prestador')) {
                print('   🚫 CANCELADA PELO PRESTADOR — não será exibida');
                return false;
              }

              // ✅ Mantém apenas canceladas pelo cliente (status = "cancelada")
              if (!status.contains('cancelada')) return false;

              // --- Verifica a data do cancelamento ---
              final canceladaEm = d['canceladaEm'] as Timestamp?;
              final atualizadoEm = d['atualizadoEm'] as Timestamp?;
              ...
              if (dataCancelamento == null) {
                print('   ❌ SEM DATA DE CANCELAMENTO - ignorado');
                return false;
              }
              ...
              final deveMostrar =
                  diferencaDias <= 3; // ⏱️ mantém por até 3 dias

              print(
                '   - Cancelada em: $dataCancelamento | Dias: $diferencaDias | Mostrar: $deveMostrar',
              );
              return deveMostrar;
            }).toList();
                        ...
                        if (effectiveStatus.startsWith('cancel'))
                          Builder(
                            builder: (_) {
                              ...
                              final dataCancelamento = _getDataCancelamento(d);
                              int diasRestantes = 0;

                              if (dataCancelamento != null) {
                                final diferenca = DateTime.now()
                                    .difference(dataCancelamento)
                                    .inDays;
                                diasRestantes = 3 - diferenca;
                                  ...
                                  Container(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        // 🔥 Aviso de remoção dinâmica
                                        Row(
                                          children: [
                                            const SizedBox(width: 4),
                                            Text(
                                              diasRestantes > 1
                                                  ? 'Este aviso será removido em $diasRestantes dias'
                                                  : diasRestantes == 1
