...
class _CadastroUsuarioState extends State<CadastroUsuario> {
  ...
  void initState() {
    ...
    // Evita chamadas reais em ambiente de teste (sem alterar app normal)
    if (Platform.environment.containsKey('FLUTTER_TEST')) {
      _geocoding = gws.GoogleMapsGeocoding(apiKey: 'FAKE_KEY');
    }
  }
  ...
  Future<void> buscarCep(String maskCep) async {
    ...
    try {
      ...
      if (logradouro.isEmpty) {
        logradouro = bairro.isNotEmpty
            ? 'Rua principal - $bairro'
      ...
      final comps = <gws.Component>[
        ...
        if (estado.isNotEmpty)
          gws.Component('administrative_area_level_1', estado),
      ...
      if (resp.status == 'OK' && resp.results.isNotEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Não foi possível posicionar o mapa. Ajuste manualmente.',
            ),
          ),
        );
      }
            ...
            children: [
              ...
              if (isPrest) ...[
                ...
                TextField(
                  ...
                  decoration: _dec(
                    '',
                    hint: 'Digite uma cidade e pressione +',
                    suffix: IconButton(
