...
import 'package:myapp/Prestador/detalhes_solicitacao.dart';
...
void main() {
  ...
  test('🟢 Deleta documento com sucesso (simulado)', () async {
    await fakeDb.collection('solicitacoesOrcamento').doc('del').set({
      'status': 'pendente',
    });
    await fakeDb.collection('solicitacoesOrcamento').doc('del').delete();
    final check = await fakeDb
        .collection('solicitacoesOrcamento')
        .doc('del')
        .get();
  ...
  // ------------------- CAMPOS E FORMATAÇÃO -------------------
  testWidgets('🧮 Formata quantidade com casas decimais corretamente', (
    tester,
  ) async {
    await fakeDb.collection('solicitacoesOrcamento').doc(docId).set({
      'status': 'pendente',
  ...
  testWidgets('🖼️ Exibe "Sem imagens anexadas" quando lista vazia', (
    tester,
  ) async {
    await fakeDb.collection('solicitacoesOrcamento').doc(docId).set({
      'status': 'pendente',
  ...
  testWidgets('💬 Exibe dados do cliente e endereço corretamente', (
    tester,
  ) async {
    await fakeDb.collection('solicitacoesOrcamento').doc(docId).set({
      'status': 'pendente',
    ...
    // 🔽 MUDANÇA AQUI: Procura pela versão formatada
    expect(find.textContaining('(62) 98888-8888'), findsOneWidget);
